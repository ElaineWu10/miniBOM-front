<template>
    ...
    <!-- 表单验证：通过 rules 属性传入约定的验证规则，并将 Form-Item 的 prop 属性设置为需校验的字段名即可 -->
		<el-form class="login-container" :rules="rules" ref="form" :model="form">
			<h1 class="title">用户登录</h1>
			<el-form-item label="用户名" prop="username">
				<el-input type="text" οnkeyup="value=value.replace(/\w.\//ig,'')" v-model="form.username" placeholder="登录账号" autocomplete="off"></el-input>
			</el-form-item>
...
export default {
    ...
    methods: {
        ...
        //             };
        //         } else {
        //             // JWT 验证失败，可能需要重新登录或者处理其他错误
        //             console.error('JWT 验证失败');
        //             this.$message.error('账号密码不正确');
        //         } 
        //     } else {
        //         // 后端没有返回 JWT，可能需要处理错误
        //         console.error('后端没有返回 JWT');
        //         this.$message.error('账号密码不正确');
        //     }
        // }).catch(function(error) {
