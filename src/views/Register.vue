<template>
	<div class="register-wrap">
		<el-form class="register-container" :rules="rules" ref="form" :model="form">
			<h1 class="title">用户注册</h1>
			<el-form-item label="用户名" prop="username">
				<el-input type="text" οnkeyup="value=value.replace(/\w.\//ig,'')" v-model="form.username" placeholder="输入用户名" autocomplete="off"></el-input>
			</el-form-item>
			<el-form-item label="密码" prop="password">
				<el-input type="password" v-model="form.password" placeholder="输入密码" autocomplete="off"></el-input>
			</el-form-item>
            <el-form-item label="电话" prop="telephone">
				<el-input type="text" v-model="form.telephone" placeholder="输入电话" autocomplete="off"></el-input>
			</el-form-item>
            <el-form-item label="邮箱" prop="email">
				<el-input type="text" v-model="form.email" placeholder="输入邮箱" autocomplete="off"></el-input>
			</el-form-item>

			<el-form-item>
				<el-button type="primary" style="width:100%;" @click="doSubmit(form)">提交</el-button>
			</el-form-item>
        ...
        var checkName=(rule,value,callback)=>{
            ...
            if(!value) callback();
                ...
                if(nameReg.test(value)){
                    callback();
                }else callback(new Error("只含数字和字母"));
            }
        };
        var checkPassword=(rule,value,callback)=>{
            let pwReg= /^[0-9a-zA-Z_?!]{1,}$/;
            if(!value) callback();
            else {
                if(pwReg.test(value)){
                    callback();
        ...
        return {
            ...
            rules: {
                username: [
                    ...
                    {min: 6, max: 32, message: "用户名6-32位", trigger: 'blur'},
